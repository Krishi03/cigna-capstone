...
import { Plus, RefreshCcw, Calendar, IndianRupee, Shield, Search, Info, XCircle } from 'lucide-react';
...
const Policies = () => {
  ...
  const filterPolicies = () => {
    ...
    const computeStatus = (p) =>
      getDaysUntilExpiry(p.endDate) <= 0 ? "Lapsed" : p.status;
  ...
  const handleRenewPolicy = (policy) => {
    if (policy.status === "Cancelled") {
      alert("Cancelled policies cannot be renewed.");
      return;
    }
  ...
  const handleCancelPolicy = async (policy) => {
    if (policy.status !== "Active") {
      alert("Only active policies can be cancelled.");
      return;
    }
    ...
    try {
      await policyService.updatePolicy(policy.policyId, {
        ...policy,
        status: "Cancelled",
      });
      fetchPolicies();
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredPolicies.map((policy) => {
            ...
            return (
              <div key={policy.policyId} className="bg-white border border-slate-300 rounded-xl hover:-translate-y-1 hover:shadow-2xl transition-all overflow-hidden">
